...
(function(){
  // i18n dictionary for stat modals
  const STAT_I18N={
    en:{
  ...
  // Wrapper to re-render stat modal texts using selected language
  const _openStatOrig = window.openStatModal || (typeof openStatModal==='function' ? openStatModal : null);
  window.openStatModal = function(key){
    ...
    }catch(e){ console.warn('stat i18n patch failed', e); }
  };

  // Delegation: open project gallery on click (robust for missing overlay)
  ...
  // Blaupunkt strict mapping (no fair/illumination pics mixed)
  ...
  function swapVolunteerIconToG(){
    try{
      ...
      if(!icon) return; icon.innerHTML='';
      const img=document.createElement('img'); img.src='./gadsdenstatecommunitycollege_logo.jpg'; img.alt='Gadsden logo'; img.className='g-badge';
      img.onerror=function(){ /* keep empty if logo missing */ };
      icon.appendChild(img);
    }catch(e){console.warn('swapVolunteerIconToG fail',e)}
  }
